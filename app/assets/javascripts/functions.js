(function($){
	...
	newsletter_form.on('submit', function(event){
		...
		var subscribe_btn    = $(this).find(".subscribe_btn");
		var refresh_loader   = $(this).find(".newsl_i .refresh_loader");
		var subscribe_output = $(this).find('#subscribe_output');
		var newsl_icon       = $(this).find(".newsl_i i");
		
		var subscribe_mail    = $(this).find(".subscribe-mail");
		var subscribe_true    = $(this).find(".newsl_i .subscribe_true");
		...
		$.ajax({
			...
			beforeSend: function() {
				...
				refresh_loader.css({"opacity" : "1"}).siblings("i").css({"opacity" : "0"});
				subscribe_output.hide(300);
			},
			success: function(data){
				subscribe_output.html(data);
				subscribe_true.css({"opacity" : "1"}).siblings("i").css({"opacity" : "0"});
				subscribe_output.show(300);
				setTimeout(function() {
					subscribe_output.hide(300);
					subscribe_btn.css({"opacity" : "1"});
					newsl_icon.css({"opacity" : "0"});
				}, 4000 );
				subscribe_mail.val('');
	...
	$(document).ready(function(){
		...
		var socials_text_color = $('.socials_text_color');
		...
		var hm_parallax = $('.hm_parallax');
		hm_parallax.each(function() {
			var $par_el = $(this);
			var $speed = ( $par_el.attr("data-speed") ) ? $par_el.attr("data-speed") : '0.5';
		...
		var entry_table = $('.entry-content table');
		entry_table.wrap('<div class="table-responsive"></div>');
		...
		var welcome_banner_slider = $('.welcome_banner_slider');
		...
		var normal_text_slider = $('.normal_text_slider');
		...
		var thumbs_gall_slider = $('.thumbs_gall_slider_con');
		thumbs_gall_slider.each(function(index, element) {
			var gall_con = $(this);
		...
		var testimonials_slider = $('.content_slider.testimonials_slider');
		testimonials_slider.each(function(index, element) {
		...
		var owl_blocks_slider = $('.owl_blocks_slider');
		...
		var related_posts_con = $('.related_posts_con');
		...
		var related_slider_wid = $('.related_slider_widget');
		related_slider_wid.owlCarousel({
			loop: true,
			smartSpeed : 500,
			autoplayTimeout : 3000,
			autoplay:true,
		...
		if (typeof SliderFx !== 'undefined' && $.isFunction(SliderFx)) {
			...
			var wobbly_slide = $('#wobbly_slide');
			var wobbly_con = $('.wobbly_slide_con');
			wobbly_slide.addClass('wobbly_slide_con');
			wobbly_con.each(function() {
		...
		if ( $.isFunction($.fn.magnificPopup) ) {
			...
			var featured_slide_block = $('.featured_slide_block');
			...
			var magnific_btn = $('.magnific-popup-btn');
			magnific_btn.magnificPopup({ 
				type: 'image',
			});
			
			var all_magnific_img = $(".magnific-popup, .img_popup:not(.gall_popup), a[data-rel^='magnific-popup']");
			all_magnific_img.magnificPopup({ 
				type: 'image',
			...
			all_magnific_img.addClass("done_magnific");
			
			var all_magnific_gall = $('.magnific-gallery, .thumbs_gall_slider_larg, .porto_galla');
			all_magnific_gall.magnificPopup({
			...
			var magnific_frames = $('.popup-youtube, .popup-vimeo, .hm_popup_frames, .hm_woocomm_vid');
			magnific_frames.magnificPopup({
				disableOn:700,
				type:'iframe',
				mainClass:'mfp-fade',
				removalDelay:160,
			...
			var popup_move = $('.popup-with-move-anim');
			popup_move.magnificPopup({
				type:'inline',
				fixedContentPos:false,
				fixedBgPos:true,
				overflowY:'auto',
		...
		if ( $.isFunction($.fn.mediaelementplayer) ) {
			...
			var hm_hosted_audio = $('audio.hosted_audio');
			var hm_hosted_video = $('video.hosted_video');
			
			hm_hosted_audio.mediaelementplayer();
			hm_hosted_video.mediaelementplayer({
		...
		var hoverdir_a = $('.porto_full_desc .hm_filter_wrapper_con > .filter_item_block > .porto_block');
		hoverdir_a.each( function() { 
			$(this).hoverdir({
		...
		var hoverdir_b = $('.has_hoverdir .featured_slide_block');
		hoverdir_b.each( function() { 
			$(this).hoverdir({
		...
		var counter_app = $('.counter');
		counter_app.appear(function() {
			$(this).find('.value_num').countTo();
		});
		
		//---------------------------------------------> Animation Progress Bars
		
		var data_progress_val = $('[data-progress-val]');
		data_progress_val.each(function() {
		...
		var hm_counter_con = $('.hm_counter_con');
		...
		var circle_progressbar = $('.hm_circle_progressbar');				
		circle_progressbar.each(function(index, element) {
			var $this_this = $(this);
		...
		var team_block_c = $('.team_block3');
		team_block_c.each(function() {
			
			var num = 0;
			var team_block_col = $(this).find('.team-col');
			
            	team_block_col.each(function(index) {
				
				var team_col_con = $(this).find('.team-col-con');
				var team_col_arr = $(this).find('.arrow');
				...
				if (typeof bg_color !== typeof undefined && bg_color !== false) {
					...
					team_col_con.css({ "background" : bg_color});
					team_col_arr.css({ "background" : bg_color});
				}
			});
        });
		...
		if ( $.isFunction($.fn.YTPlayer) ) {
			...
			var youtube_bg_video = $('.youtube_bg_video');
		...
		var html_video_background = $('.html_video_background');
		...
		var your_rate = $('.your_rate');
		your_rate.each(function(index, element) {
		  	var score = $(this).find('.outline_stars').data('rate');
			var outline_stars = $(this).find('.outline_stars');
			outline_stars.css({width : score+'%'});
		});
	     
		var rating_preloader = $('.comment-form-rating .stars a');
		rating_preloader.on("click", function() {
			
			var data_rel = $(this).attr("data-rate");
			var select_rating = $('select#rating');
			
        		$(this).addClass('active').siblings().removeClass('active');
			select_rating.val(data_rel);
			return false;
        	});
		...
		if ( $.isFunction($.fn.noUiSlider) ) {
			...
			var shop_price_slider = $('#shop_price_slider');
			...
			var shop_lower = $('#shop_price_slider_lower');
			var min_price = $('#min_price');
			var shop_upper = $('#shop_price_slider_upper');
			
			shop_price_slider.Link('lower').to( shop_lower, null, wNumb({
				prefix: '$',
			}));	
			shop_price_slider.Link('lower').to( min_price, null, wNumb({}));	
			
			shop_price_slider.Link('upper').to( shop_upper, null, wNumb({
				prefix: '$',
			}));
			shop_price_slider.Link('upper').to( min_price, null, wNumb({}));
		}
		
	     //---------------------------------------------> Animated
	    
	     var css_animated = $('.animated');
		css_animated.appear(function() {
			var elem = $(this);
		...
		var ease_scroll = $('.scroll');
		ease_scroll.on('click', function(event) {
			var $anchor = $(this);
		...
		var quantity_controll = $('.quantity_controll');
		...
		function hmtabswidth(){
			var hm_tabs = $('.hm-tabs');
			hm_tabs.each(function(index) {
				var $allparent = $(this);
				var all_width = $allparent.width();
				var all_lis = 0;
				var navi_tabs_li = $allparent.find(".tabs-navi > li");
				
				navi_tabs_li.each(function(index, element) {
			...
			sort_options.each(function(index) {
				...
				var all_width = $allparent.width();
				var all_lis = 0;
				var filter_by_li = $allparent.find("#filter-by > li");
				
				filter_by_li.each(function(index, element) {
                    	var li_width = $(this).outerWidth();
					all_lis += li_width;
                	});
				if(all_lis >= all_width){
		...
		var main_hm_tabs = $('.hm-tabs');
		main_hm_tabs.each(function(index) {
			var allparent = $(this);
			var all_width = allparent.width();
		...
		var hm_filter_wrapper = $('.hm_filter_wrapper');
		hm_filter_wrapper.each(function(index, element) {
			var main_parent = $(this);
			var filter_buttons  = main_parent.find('#filter-by');
			var filtered_parent  = main_parent.find('.hm_filter_wrapper_con');
			var filter_buttons_a  = main_parent.find("#filter-by li a");
			
			var col_width = main_parent.hasClass("masonry_porto") ? 1 : 0;
			
			filter_buttons_a.each(function(index, element) {
				var filter_button_num = $(this).find(".num");
				var get_class = $(this).attr("data-option-value");
				var lenghty = main_parent.find(".hm_filter_wrapper_con > "+get_class).length;
				filter_button_num.html(lenghty);
			});
			...
			var $container = main_parent.find('.hm_filter_wrapper_con').imagesLoaded( function() {
				...
				function  timeline_on_left_and_right(){
					var timeline_blocks = $('.hm_filter_wrapper_con.timeline').children('.filter_item_block');
					timeline_blocks.each(function(index, element) {
						var last_child = $(this);
		...
		$(window).on("resize", function() {
			...
			var animated_resize = $('.animated');
			animated_resize.each(function(index, element) {
				var elem = $(this);
				if ( elem.hasClass('visible') ) {
		...
		var top_cart_search = $('.hm_icon_search > a, .top_add_card');
		top_cart_search.on('click', function(event){ 
		     var parent = $(this).parent();
			var $this_btn = $(this);
			var $target_block = $this_btn.siblings('div');
			var all_top_cart_search = $('#logo, #main_nav, .top_add_card, .hm_icon_search_btn');
			var hm_search_c = $('.hm_search_c');
			...
			if(parent.hasClass('active') && $target_block.hasClass('hm_active_prep')){
				...
				if(parent.hasClass('hm_icon_search')){
					all_top_cart_search.css({"opacity" : 0});
				}
				
				var hm_toogle_prep = $('.hm_toogle_prep');
				hm_toogle_prep.removeClass('hm_active_prep');
				$target_block.addClass('hm_active_prep');
				
				var hm_toogle_prep_b = $('.hm_toogle_prep:not(.hm_active_prep)');
				hm_toogle_prep_b.fadeOut();
			...
			$(document).on("click touchstart", function(e){
				...
				if(parent.hasClass('hm_icon_search')){
					all_top_cart_search.css({"opacity" : 1});
				}
			});
